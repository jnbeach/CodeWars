...
namespace WhichXForThatSum
{
    class Program
    {
        ...
        {
            double solution1 = ((2 * m + 1) + Math.Sqrt(4 * m + 1)) / (2 * m);
            double solution2 = ((2 * m + 1) - Math.Sqrt(4 * m + 1)) / (2 * m);
            if (solution1 > 0 && solution1 < 1)
            {
                return solution1;
            }
            else if (solution2 > 0 && solution2 < 1)
            {
                return solution2;
            }
            return 0;

            // Brute Force Trial method. Guesses solutions with a for loop
            // decimal trial;
